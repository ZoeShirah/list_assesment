...
def every_other_item(items):
    ...
    items = [items[x] for x in range(len(items)) if x % 2 == 0]
...
def largest_n_items(items, n):
    ...
    """
    items.sort()
    n = n+1  # we will use the n passed in to reference the index that we want,
             # but the way list slicing is indexed it needs to be one higher
